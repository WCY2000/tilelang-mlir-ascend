...
def run_single_shape(shape, log_dir: Path):
    ...
    with open(log_file, "w") as f, redirect_stdout(f), redirect_stderr(f):
        ...
        try:
            ...
            key_args, _ = list(elementwise_abs._tuner_cache.keys())[-1]
            provided_args = {
                name: val
                for (name, val) in zip(
                    inspect.signature(elementwise_abs.jit_impl.func).parameters,
                    key_args,
                    strict=False,
                )
            }
...
def main():
    root_log_dir = Path("./abs_multikernel_shape_logs_2d")
