...
def run_single_shape(shape, log_dir: Path):
    ...
    with open(log_file, "w") as f, redirect_stdout(f), redirect_stderr(f):
        ...
        try:
            ...
            # ----------------------------------------------------------------
            # We inspect the *outer* generator function so the analyser sees
            # the full body including the inner prim_func definition.
            # Pass a hints dict here if you want to override auto-detection,
            # e.g. hints={"kernel_type": "mix"}.
            kernel_type = analyze_kernel_type(elementwise_add)
            print(f"\nKernel type analysis result: {kernel_type!r}")
            print("-" * 40)
            # ----------------------------------------------------------------
